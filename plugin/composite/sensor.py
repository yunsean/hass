...
SENSOR_SCHEMA = vol.Schema({
    ...
    vol.Required(CONF_ATTR_SENSOR): vol.All(cv.ensure_list)
})
...
def async_setup_platform(hass, config, async_add_entities,
    ...
    for device, device_config in config[CONF_SENSORS].items():
        ...
        attr_sensors = device_config.get(CONF_ATTR_SENSOR)        
        
        attrSensorMap = {}
        for sensor in attr_sensors:
            parts = sensor.split(':', 1)
            if len(parts) > 1:
                attrSensorMap[parts[1]] = parts[0]
...
class SensorTemplate(Entity):
    ...
    def __init__(self, hass, device_id, friendly_name, friendly_name_template,
                 ...
                 entity_picture_template, entity_ids, attrSensorMap, device_class):
        ...
        self._attrSensorMap = attrSensorMap
        self._attr = {}
    ...
    def async_added_to_hass(self):
        ...
        def template_sensor_state_listener(entity, old_state, new_state):
            ...
            if entity in self._attrSensorMap:
                self._attr[self._attrSensorMap[entity]] = new_state.state                
        ...
        def template_sensor_startup(event):
            ...
            for entity in self._attrSensorMap:
                self._entities.append(entity)
                if self.hass.states.get(entity):
                    self._attr[self._attrSensorMap[entity]] = self.hass.states.get(entity).state     
            async_track_state_change(self.hass, self._entities, template_sensor_state_listener)
            self.async_schedule_update_ha_state(True)
