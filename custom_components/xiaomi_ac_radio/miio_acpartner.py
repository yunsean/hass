...
def async_setup_platform(hass, config, async_add_devices, discovery_info=None):
    ...
    xiaomi_miio_radio = XiaomiMiioRadio(friendly_name, device, unique_id, apiKey, secretKey, speed, pitch, volume, person, baseUrl, basePath, notify)
...
class BaiduTTS:
    def __init__(self, apiKey, secretKey, speed = 5, pitch = 5, volume = 15, person = 0):
        self.apiKey = apiKey
...
class XiaomiMiioRadio(RadioDevice):
    def __init__(self, friendly_name, device, unique_id, apiKey, secretKey, speed, pitch, volume, person, baseUrl, basePath, notify):
    ...
    def update_state(self):
        status = self.is_on
        attrs = self.device_state_attributes
        self.hass.states.async_set(DOMAIN + "." + self._name, "on" if status else "off", attrs)
    ...
    def async_play_tts(self, message, **kwargs):
        ...
        try:
            if (self._baidu == None) and (self._apiKey == None or self._secretKey == None):
                ...
                return False
            if self._baidu == None:
                self._baidu = BaiduTTS(self._apiKey, self._secretKey, self._speed, self._pitch, self._volume, self._person)
            if not self._baidu.generate_tts(message, self._mp3Path):
